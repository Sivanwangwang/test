...
def gene_len(list_gene):
    ...
    with open("gene_len.txt", 'w') as fp_out:
        fp_out.write('chr'+'\t'+'gene_id'+'\t'+'type'+'\t'+'start'+'\t'+'end'+'\t'+'length'+'\n')
        for gene_id, info in list_gene.items():
            len = info.end - info.start + 1                 #用list_gene字典中的值gene的属性end和start
            fp_out.write("\t".join([info.chr,gene_id,info.catag,str(info.start),str(info.end),str(len)]) + "\n")
...
def gene_transcript1(list_transcript):
    ...
    for info in list_transcript.values():
        ...
        if gene_id not in count_transcript:     #遇到一个新的gene_id就建立一个新的字典作为第一层字典的‘健’
            ...
            count_transcript[gene_id][chr] = [transcript_id]   #并把第一个元素（transcript_id)加到列表‘值’中
        else:
            count_transcript[gene_id][chr].append(transcript_id)  #遇到了一个新的(transcript_id)加到列表‘值’中
    with open("gene_transcript1.txt", 'w') as fp_out:
        fp_out.write('chr'+'\t'+'gene_id'+'\t'+'transcript_id'+'\t'+'num'+'\n')
        for gene_id, name in count_transcript.items():
            for chr,transcript_id in name.items():
                # print("\t".join([gene_id, str(num)]) + "\n")
                transcript_id_set = "|".join(transcript_id)
                fp_out.write("\t".join([chr,gene_id,transcript_id_set,str(len(transcript_id))]) + "\n")
...
def gene_exon_pos1(list_gene, list_transcript, list_exon):
    ...
    for exon in list_exon:
        ...
        if transcript_id in count_exon:
            ...
            count_exon[transcript_id] = "%s-%s" % (str(exon.start), str(exon.end))
    print("The length of count_exon:"+str(len(count_exon.keys())))                             #打印转录本的个数，可以与后边的合成字典的键值对进行对比，防止遗漏
    for transcript_id,transcript in list_transcript.items():
        gene_id = transcript.parent
        chr = transcript.chr
        #name = transcript.name
        catag = transcript.catag
        all = "\t".join([str(chr),catag,gene_id])
        if all not in count_transcript:                     #为了避免只有一个元素，所以要先判定不在字典里就新建一对健值，然后把唯一元素放入值的列表中
    ...
    with open("gene_transcript_exon1.txt", 'w') as fp_out:
        fp_out.write("Chr"+"\t"+"Type"+"\t"+"Gene_ID"+"\t"+"Transcript_ID"+"\t"+"exon_pos"+"\t"+"exon_number"+"\n")   #加个列名
        for gene_id, transcript in gene_transcript_exon.items():
            for transcript_id, exon_pos in transcript.items():
